...
import importlib
import pkgutil
from gciunet.paths import network_training_output_dir, preprocessing_output_dir, default_plans_identifier
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None,folder=None):
    """
    This is a utility function to load any GCI_UNet trainer from a pkl. It will recursively search
    gciunet.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    is specified, it will furthermore load the checkpoint file in train/test mode (as specified by train).
    The pkl file required here is the one that will be saved automatically when calling gciunetTrainer.save_checkpoint.
    :param pkl_file:
    :param checkpoint:
    :param train:
    ...
    if 'nnUNet' in name:
        name=name.replace('nnUNet','nnFormer','GCI_UNet')
    if len(init)>10:
        init=list(init)
        del init[2]
        del init[-2]
    search_in = join(gciunet.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="gciunet.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in gciunet.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within gciunet.trainig.network_training (search is recursive)."
...
if __name__ == "__main__":
    pkl = "/srv/qiaoqiang/gci_unet/DATASET_Tumor/gci_unet_trained_models/gci_unet/3d_fullres/Task003_tumor/gci_unet_trainer_tumor__gci_unet_Plansv2.1/fold_1/model_best.model.pkl"
    checkpoint = pkl[:-4]
    train = False
