...
import argparse
from gciunet.preprocessing.preprocessing import resample_data_or_seg
from batchgenerators.utilities.file_and_folder_operations import maybe_mkdir_p
import gciunet
from gciunet.run.default_configuration import get_default_configuration
...
if __name__ == "__main__":
    ...
    trainer_class = recursive_find_python_class([join(gci_unet.__path__[0], "training", "network_training")],
                                                trainerclass,
                                                "gci_unet.training.network_training")

    if trainer_class is None:
        raise RuntimeError("Could not find trainer class in gci_unet.training.network_training")
    else:
        assert issubclass(trainer_class,
                          Trainer_acdc), "network_trainer was found but is not derived from gci_unetTrainer"
