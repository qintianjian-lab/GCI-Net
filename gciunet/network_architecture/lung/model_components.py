...
from gciunet.network_architecture.dynunet_block import get_conv_layer, UnetResBlock,EmbeddingLung,SeparableConv3d
...
class GCIUNetEncoder(nn.Module):
    def __init__(self, dims=[32, 64, 128, 256], msca_dim=[32, 64, 128], spatial_dims=3, in_channels=1, dropout=0.0):
        super().__init__()

        self.downsample_layers = nn.ModuleList()  # stem and 3 intermediate downsampling conv layers
        stem_layer = nn.Sequential(
            EmbeddingLung(in_channels=in_channels,out_channels=dims[0]),
...
class GlobalGuidedFusion(nn.Module):
    ...
    def __init__(
        ...
        if pos_embed:
            ...
            self.global_pos_embed = nn.Parameter(torch.zeros(1, 144, 256))
    ...
    def forward(self, skip, out, global_fea):
        ...
        if self.pos_embed is not None:
            ...
            # print('global_fea.shape',global_fea.shape)
            # print('self.global_pos_embed.shape',self.global_pos_embed.shape)
...
class GlobalGuidedLayer(nn.Module):
    ...
    def __init__(self, input_size, gf_proj_size, skip_proj_size, num_heads, attn_drop=0.1):
        ...
        self.EF2 = nn.Parameter(init_(torch.zeros(144, gf_proj_size)))  # 针对 global_fea，N 为4*4*4
        self.attn_drop = nn.Dropout(attn_drop)

    def forward(self, skip, out, global_fea):

        B, N, C = skip.shape
        B1, N1, C1 = global_fea.shape  # (1,144,256)
