...
from gciunet.experiment_planning.common_utils import split_4d_nifti
from gciunet.paths import gci_unet_raw_data, gci_unet_cropped_data, preprocessing_output_dir
from gciunet.preprocessing.cropping import ImageCropper
...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    ...
    output_folder = join(gci_unet_raw_data, "Task%03.0d_" % overwrite_task_output_id + task_name)
...
def crop(task_string, override=False, num_threads=default_num_threads):
    cropped_out_dir = join(gci_unet_cropped_data, task_string)
    ...
    splitted_4d_output_dir_task = join(gci_unet_raw_data, task_string)
    lists, _ = create_lists_from_splitted_dataset(splitted_4d_output_dir_task)

    imgcrop = ImageCropper(num_threads, cropped_out_dir)
    imgcrop.run_cropping(lists, overwrite_existing=override)
    shutil.copy(join(gci_unet_raw_data, task_string, "dataset.json"), cropped_out_dir)
...
def plan_and_preprocess(task_string, processes_lowres=default_num_threads, processes_fullres=3, no_preprocessing=False):
    from gciunet.experiment_planning.experiment_planner_baseline_2DUNet import ExperimentPlanner2D
    from gciunet.experiment_planning.experiment_planner_baseline_3DUNet import ExperimentPlanner
    ...
    shutil.copy(join(gci_unet_raw_data, task_string, "dataset.json"), preprocessing_output_dir_this_task_train)
