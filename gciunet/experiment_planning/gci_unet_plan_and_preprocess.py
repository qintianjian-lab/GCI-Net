...
from gciunet.experiment_planning.utils import crop
from gciunet.paths import *
import shutil
from gciunet.utilities.task_name_id_conversion import convert_id_to_task_name
from gciunet.preprocessing.sanity_checks import verify_dataset_integrity
...
def main():
    ...
    for i in task_ids:
        ...
        if args.verify_dataset_integrity:
            verify_dataset_integrity(join(gci_unet_raw_data, task_name))

        crop(task_name, False, tf)

        tasks.append(task_name)

    search_in = join(gciunet.__path__[0], "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d, current_module="gciunet.experiment_planning")
        if planner_3d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "gciunet.experiment_planning" % planner_name3d)
    else:
        planner_3d = None

    if planner_name2d is not None:
        planner_2d = recursive_find_python_class([search_in], planner_name2d, current_module="gciunet.experiment_planning")
        if planner_2d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "gciunet.experiment_planning" % planner_name2d)
    else:
        planner_2d = None
