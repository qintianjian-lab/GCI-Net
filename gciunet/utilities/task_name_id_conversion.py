...
from gciunet.paths import gci_unet_raw_data, preprocessing_output_dir, gci_unet_cropped_data, network_training_output_dir
...
def convert_id_to_task_name(task_id: int):
    ...
    if gci_unet_raw_data is not None:
        candidates_raw = subdirs(gci_unet_raw_data, prefix=startswith, join=False)
    else:
        candidates_raw = []

    if gci_unet_cropped_data is not None:
        candidates_cropped = subdirs(gci_unet_cropped_data, prefix=startswith, join=False)
    else:
        candidates_cropped = []
    ...
    if len(unique_candidates) > 1:
        raise RuntimeError("More than one task name found for task id %d. Please correct that. (I looked in the "
                           "following folders:\n%s\n%s\n%s" % (task_id, gci_unet_raw_data, preprocessing_output_dir,
    ...
    if len(unique_candidates) == 0:
        raise RuntimeError("Could not find a task with the ID %d. Make sure the requested task ID exists and that "
                           ...
                           "variables." %
                           (task_id,
                            os.environ.get('gci_unet_preprocessed') if os.environ.get('gci_unet_preprocessed') is not None else 'None',
                            os.environ.get('gci_unet_RESULTS_FOLDER') if os.environ.get('gci_unet_RESULTS_FOLDER') is not None else 'None',
                            os.environ.get('gci_unet_raw_data_base') if os.environ.get('gci_unet_raw_data_base') is not None else 'None',
                            ))
